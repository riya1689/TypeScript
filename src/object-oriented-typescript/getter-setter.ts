// getter
// setter

class BankAccount {
  public readonly userId: number;
  public userName: string;
  private _userBalance: number;

  constructor(userId: number, userName: string, userBalance: number) {
    this.userId = userId;
    this.userName = userName;
    this._userBalance = userBalance;
  }

  //    balance k set krtese
  //   addBalance(balance: number) {
  //    return this._userBalance = this._userBalance + balance;
  //   }

  // setter use kre krte chai

  set addBalance(amount: number) {
    this._userBalance = this._userBalance + amount;
  }

  //   get krbo

  //   getBalance() {
  //     return this._userBalance;
  //   }

  // getter use kore get korte chai
  get getBalance() {
    return this._userBalance;
  }
}

const riyaAccount = new BankAccount(111, "Mezba", 20);

// riyaAccount.addBalance(100); // function call krte hsse
// riyaAccount.addBalance(60);

// console.log(riyaAccount.getBalance()); // function call krte hsse

riyaAccount.addBalance = 100;
riyaAccount.addBalance = 60;

console.log(riyaAccount.getBalance);
// console.log(riyaAccount);